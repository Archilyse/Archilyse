...
class TestSwissTopoMountainsTriangulator:
    ...
    def test_raster_window_provider(self, mocker, bounds, region):
        import surroundings.swisstopo.raster_window_triangulator as rwt

        spy_raster_window_provider = mocker.spy(
            rwt, "SwissTopoMountainsRasterWindowProvider"
        )
        triangulator = rwt.SwissTopoMountainsTriangulator(
...
class TestSwissTopoGroundsTriangulator:
    @pytest.mark.parametrize("bounds", [(0, 0, 1, 1), (1, 1, 2, 2)])
    @pytest.mark.parametrize("region", [REGION.CH, REGION.LAT_LON])
    def test_raster_window_provider(self, mocker, bounds, region):
        import surroundings.swisstopo.raster_window_triangulator as rwt
        ...
        spy_raster_window_provider = mocker.spy(
            rwt, "SwissTopoGroundsRasterWindowProvider"
        )
        triangulator = rwt.SwissTopoGroundsTriangulator(
            bounding_box=box(*bounds),
            region=region,
        )
