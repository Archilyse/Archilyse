...
const Breadcrumb = ({ hierarchy = [] }) => {
  ...
  return (
    <Breadcrumbs
      ...
      separator={<Icon style={{ marginLeft: 0, fontSize: 20 }}>navigate_next_rounded</Icon>}
    >
      {currentHierarchy.map((link, index) => {
        if (index === currentHierarchy.length - 1)
          return (
            <a key={link.text} className="breadcrumb-link">
              <Truncate maxWidth={maxWidth}>{link.text}</Truncate>
              <Icon style={{ marginLeft: 4, fontSize: '20px' }}>keyboard_arrow_down_rounded</Icon>
            </a>
          );
