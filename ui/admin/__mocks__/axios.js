...
const MOCK_BUILDINGS_BY_SITE = require('./entities/buildings_by_site');
...
const URL_REGEXPS = {
  ...
  BUILDINGS_BY_SITE: /^\/building\/\?site_id=[0-9]+$/,
  SITE: /^\/site\/[0-9]+$/,
  CLIENT: /^\/client\/[0-9]+$/,
...
module.exports = {
  get: jest.fn(url => {
    ...
    if (BUILDINGS_BY_SITE.test(url)) {
      return Promise.resolve({
        data: MOCK_BUILDINGS_BY_SITE,
      });
    }
    if (FLOOR.test(url)) {
      return Promise.resolve({
